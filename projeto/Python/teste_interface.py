...
##############################################################################
    def import_img(self, path, padrao_cor = None, normalizacao = True):
    	img = cv2.imread(path)
    	if padrao_cor == "cinza":
...
Menu_Arquivo.add_command(label = "abrir imagem", command = lambda: App_I.import_img(App_I, path+path_image, "RGB", True))
